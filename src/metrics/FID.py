...
def load_image(path):
    ...
    im = cv2.imread(impath)
    W, H = im.shape[:2]
    final_images = np.zeros((1, H, W, 3), dtype=im.dtype)
    im = im[:, :, ::-1] # Convert from BGR to RGB
    assert im.dtype == final_images.dtype
    final_images[0] = im
    return final_images
...
if __name__ == "__main__":
    ...
    result=[]
    for a in method:
        predict_video_file=f'result/{a}/result/fake/0.mp4'
        pre_reader = imageio.get_reader(predict_video_file)
        predict_driving_video=np.array([cv2.cvtColor(im,cv2.COLOR_RGB2BGR) for im in pre_reader])
        pre_reader.close()

        
        predict_video_file=f'result/{a}/result/real/0.mp4'
        pre_reader = imageio.get_reader(predict_video_file)
        gt_driving_video=np.array([cv2.cvtColor(im,cv2.COLOR_RGB2BGR)  for im in pre_reader])
        pre_reader.close()

        ssim_pc_avs=calculate_fid(predict_driving_video, gt_driving_video, use_multiprocessing=False, batch_size=12)
        result.append(ssim_pc_avs)
