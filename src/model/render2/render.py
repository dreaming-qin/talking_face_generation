...
class StyleGAN2Generator(BaseNetwork):
    def __init__(
        ...
        self.mapping_net = MappingNet(**mapping_cfg)
...
if __name__=='__main__':
    ...
    render=StyleGAN2Generator(opt,config['mapping_net'],style_dim=style_dim).to(device)

    input_img=torch.rand((10,3,256,256)).to(device)
    style=torch.rand((10,73,27)).to(device)

    fake_img,_=render(input_img,style)
    a=1
