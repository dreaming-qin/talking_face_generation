...
if __name__ == '__main__':
    syncnet = LandmarkHubertSyncNet(lm_dim=60)
    hubert = torch.rand(2, 10, 1024)
    lm = torch.rand(2, 5, 60)
    ...
    # 加载模型
    state_dict=torch.load('checkpoints/model_ckpt_steps_40000.ckpt')
    syncnet.load_state_dict(state_dict['state_dict']['model'])
    mel_embedding, exp_embedding = syncnet(hubert, lm)
    label = torch.tensor([1., 0.])
    loss = syncnet.cal_sync_loss(mel_embedding, exp_embedding, label)
    print("{}".format(loss))