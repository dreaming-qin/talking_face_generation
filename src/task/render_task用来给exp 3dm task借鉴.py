...
def eval(exp_3dmm_model,render,dataloader,loss_fun,checkpoint=None):
    ...
    for data in tqdm(dataloader):
        ...
        # [B,len,win size,73]
        driving_source=get_windows_by_repeat(driving_source,config['render_win_size'])
...
def save_result(exp_3dmm_model,render,dataloader,save_dir,save_video_num):
    ...
    for data in dataloader:
        ...
        # test
        # [1,20,win size,73]
        driving_source=driving_source[:,13:20]
        data['raw_video']=data['raw_video'][:,13:20]
        data['video_input']=data['video_input'][:,13:20]
        data['img']=data['img'][:,13]
        # test
        data['img']=data['img'][:,10:-10,105:-70].permute(0,3,1,2)
        resize = transforms.Resize([256,256])
        data['img'] = resize(data['img'])
        data['img']=data['img'].permute(0,2,3,1)
        data['raw_video']=data['raw_video'][:,:,10:-10,105:-70].permute(0,1,4,2,3)
        resize = transforms.Resize([256,256])
        data['raw_video'] = resize(data['raw_video'][0]).unsqueeze(0)
        data['raw_video']=data['raw_video'].permute(0,1,3,4,2)
        ...
        for real_video,video_input,img,warp,fake_video in zip(data['raw_video'],data['video_input'],data['img'],warp_img,output_imgs):
            ...
            imageio.mimsave(save_path,(video*255.0).astype('uint8') ,fps=5)
    render.train()
...
def run(config):
    ...
    train_dataset=RenderDataset(config,type='train',max_len=54)
    ...
    # 验证集
    eval_dataset=RenderDataset(config,type='eval',max_len=50)
    ...
    # 测试集
    test_dataset=RenderDataset(config,type='test',max_len=50)
    ...
    # test
    save_result(exp_model,render,eval_dataloader,'result//warp/epoch_-1',save_video_num=10)
    ...
    train_logger.info('准备完成，warping 网络开始训练')
    best_loss=int(1e10)
    # 开始训练，先训练warp阶段
    # 优化器
    params=[]
    for name,param in render.named_parameters():
        if 'editing_net' not in name:
            params.append(param)
    optimizer = torch.optim.Adam(params, lr=config['lr'], betas=(0.5, 0.999))
    ...
    for epoch in range(config['warp_epoch']):
        epoch_loss=0
        for data in tqdm(train_dataloader):
            ...
            # test
            # [1,2,win size,73]
            data['img']=data['img'][:,:10]
            driving_source=driving_source[:,13:23]
            data['raw_video']=data['raw_video'][:,13:23]
            # test
            data['img']=data['img'][:,:,10:-10,105:-70].permute(0,1,4,2,3)
            resize = transforms.Resize([256,256])
            data['img'] = resize(data['img'][0]).unsqueeze(0)
            data['img']=data['img'].permute(0,1,3,4,2)
            data['raw_video']=data['raw_video'][:,:,10:-10,105:-70].permute(0,1,4,2,3)
            resize = transforms.Resize([256,256])
            data['raw_video'] = resize(data['raw_video'][0]).unsqueeze(0)
            data['raw_video']=data['raw_video'].permute(0,1,3,4,2)
            ...
            input_image=[]
            # [B,len,3,H,W]
            input_image=data['img'].permute(0,1,4,2,3)

           
            warp_img=[]
            for drving,img in zip(driving_source,input_image):
                output = render(img, drving,stage='warp')
                # output_dict['fake_image'] [len,3,H,W]
                warp_img.append(output['warp_image'])
            # [B,len,3,H,W]
            warp_img=torch.stack(warp_img)


            # 计算loss
            loss=loss_function(warp_img.permute(0,1,3,4,2),data,stage='warp')
            ...
            optimizer.step()
        train_logger.info(f'对warp网络，第{epoch}次迭代获得的loss值为{epoch_loss}')
        scheduler.step(epoch_loss)
        train_logger.info('当前学习率为{}'.format(optimizer.param_groups[0]['lr']))
        if epoch_loss<best_loss:
            save_result(exp_model,render,eval_dataloader,'result//warp/epoch{}'.format(epoch),save_video_num=10)
            best_loss=epoch_loss
            pth_path= os.path.join(config['checkpoint_dir'],f'warp_epoch_{epoch}_loss_{epoch_loss}.pth')
    ...
    best_loss=int(1e10)
    best_checkpoint=0
    train_logger.info('warping 网络训练结束，训练editing网络')
    # 再训练editting阶段
    # 优化器
    params=[]
    for name,param in render.named_parameters():
        if 'editing_net' in name:
            params.append(param)
    optimizer = torch.optim.Adam(params, lr=config['lr'], betas=(0.9, 0.999))
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.5,verbose=True,cooldown=0,
        patience=config['lr_scheduler_step'],mode='min', threshold_mode='rel', threshold=0, min_lr=1e-06)
    for epoch in range(config['edit_epoch']):
        epoch_loss=0
        for data in tqdm(train_dataloader):
            # 把数据放到device中
            for key,value in data.items():
