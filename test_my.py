...
if __name__=='__main__':
    # 将256*256的视频与原始音频合并
    video_list=glob.glob('/workspace/code/EAMM-main/data/emo/*.mp4')
    video_list=sample(video_list, 15)
    for video in video_list:
        str_list=os.path.basename(video).split('_')
        iden=str_list[0]
        angle=str_list[1]
        emo=str_list[2]
        level=f'{str_list[3]}_{str_list[4]}'
        mp4=str_list[5]
        orginal_path=f'/workspace/dataset/MEAD/{iden}/video/{angle}/{emo}/{level}/{mp4}'
        cmd ='ffmpeg -i {} -i {} -loglevel error -c copy -map 0:0 -map 1:1 -y -shortest {}'.format(
            video,orginal_path,f'temp/{os.path.basename(video)}')
        os.system(cmd) 
