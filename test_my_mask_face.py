...
if __name__=='__main__':
    ...
    pkl_list=sorted(glob.glob(f'data_mead/format_data/*/*/*.pkl'))
    for pkl_file in tqdm(pkl_list):
        ...
        pred_mask, _, pred_face = render(pred_vertex, facemodel.face_buf, feat=pred_color)

        # img=img_format.format(data).to(device)
        pred_mask=mask_format.format(pred_mask,data)
        ...
        os.makedirs(os.path.dirname(pkl_file.replace('data_mead','data_mead_fake_face')),exist_ok=True)
        with open(pkl_file.replace('data_mead','data_mead_fake_face'),'wb') as f:
            f.write(info)

        
        # # 解压测试
        # with open(pkl_file.replace('data_mead','data_mead_fake_face'),'rb') as f:
        #     byte_file=f.read()
        # byte_file=zlib.decompress(byte_file)
        # data= pickle.loads(byte_file)
        # video=data['path']
